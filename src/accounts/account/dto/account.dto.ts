...
export class CreateAccountDto {
  ...
  category: string;       // e.g. "Asset", "Liability", "Equity", "Revenue", "Expense"
    ...
    @ApiProperty({ example: 'Current Asset', description: 'Sub-category of the account' })
  @IsString()
  @IsNotEmpty()
  subCategory: string;    // e.g. "Current Asset", "Operating Expense", "Sales Revenue"

    @ApiProperty({ example: 'Main cash account for daily transactions', description: 'Description of the account' })
  @IsString()
  @IsNotEmpty()
  description: string;
  ...
  debit?: number;
    ...
    @ApiProperty({ example: '2024-12-31', description: 'Opening date or last transaction date' })
  @IsDateString()
  @IsOptional()
...
export class AccountResponseDto {
    @ApiProperty({ example: 'acc_abc123', description: 'Unique identifier for the account' })
  id: string;
    @ApiProperty({ example: 'Cash on Hand', description: 'Name of the account' })
  name: string;
    @ApiProperty({ example: '1100', description: 'Account code or ID' })
  ...
  type: string;
    @ApiProperty({ example: 1500, description: 'Current balance of the account' })
  balance: number;
    @ApiProperty({ example: 500, description: 'Current debit balance of the account' })
  credit?: number | null;
    @ApiProperty({ example: 200, description: 'Current credit balance of the account' })
  debit?: number | null;
    @ApiProperty({ example: '2024-12-31', description: 'Opening date or last transaction date' })
  ...
  entityId: string;
    @ApiProperty({ example: '2024-01-01T12:00:00.000Z', description: 'Account creation timestamp' })
