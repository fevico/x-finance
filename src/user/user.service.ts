...
export class UserService {
  ...
  async createUser(groupId: string, userId: string, dto: CreateUserDto) {
    ...
    if (!dto.email && !dto.emails) {
      throw new BadRequestException('Either email (single) or emails (bulk) is required');
    }
    ...
    if (!role || role.groupId !== groupId) {
      throw new NotFoundException('Role not found or does not belong to this group');
    }

    // Determine systemRole based on scope
    const systemRoleValue = dto.scope === 'ENTITY' ? systemRole.user : systemRole.admin;
  ...
  private async createBulkUsers(
    ...
    const check = await this.subscriptionService.checkUserLimit(groupId, emailList.length);
    if (!check.allowed) {
      throw new ForbiddenException(check.message);
    }
    ...
    // Increment subscription user count
    await this.subscriptionService.incrementUserCount(groupId, createdUsers.length);
    ...
    return {
      ...
      message: options.sendWelcomeEmail
        ? `${createdUsers.length} users created. Welcome emails queued.`
        : `${createdUsers.length} users created. No emails sent.`,
    };
  }
  ...
  async updateUser(userId: string, groupId: string, dto: UpdateUserDto) {
    ...
    if (dto.requirePasswordChange !== undefined) updateData.requirePasswordChange = dto.requirePasswordChange;
