...
export class CacheService {
  ...
  async publishInvalidation(channel: string, key: string): Promise<void> {
    try {
      await this.redis.publish(channel, JSON.stringify({ key, timestamp: Date.now() }));
    } catch (error) {
  ...
  async invalidateUserCache(userId: string, groupId: string): Promise<void> {
    ...
    await this.publishInvalidation(`user:${groupId}:invalidate`, `user:${userId}`);
  }

  /**
   * Invalidate entity data (when entity changes/is deleted)
   */
  async invalidateEntityCache(groupId: string, entityId: string): Promise<void> {
    await Promise.all([
    ...
    await this.publishInvalidation(`entity:${groupId}:invalidate`, `entity:${entityId}`);
  }
  ...
  async invalidateGroupEntities(groupId: string): Promise<void> {
    ...
    // Publish invalidation event
    await this.publishInvalidation(`group:${groupId}:entities-invalidate`, `entities:${groupId}`);
  }
  ...
  async invalidateGroupModuleCache(groupId: string): Promise<void> {
    ...
    // Publish invalidation event for real-time sync
    await this.publishInvalidation(`group:${groupId}:invalidate`, `modules:${groupId}`);
  }
}
