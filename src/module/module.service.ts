...
export class ModuleService {
  ...
  async getAllModules(moduleVersion?: number): Promise<any[]> {
    ...
    const cacheKey = moduleVersion ? `${cacheKeyBase}:v${moduleVersion}` : cacheKeyBase;
   ...
   */
  async getModulesByScope(scope: ModuleScope, moduleVersion?: number): Promise<any[]> {
   ...
   */
  async getModuleByKey(moduleKey: string, moduleVersion?: number): Promise<any> {
  ...
  async createModule(createModuleDto: CreateModuleDto): Promise<any> {
    const { moduleKey, displayName, description, scope = ModuleScope.ENTITY, menu, isMenuVisible = true } = createModuleDto;
   ...
   */
  async updateModule(moduleId: string, updateModuleDto: UpdateModuleDto): Promise<any> {
    ...
    // Invalidate all module caches by incrementing version
    await this.cacheInvalidation.incrementModuleVersion();
    console.log(`✅ Module updated: ${updatedModule.moduleKey} (ID: ${moduleId})`);
  ...
  async deleteModule(moduleId: string): Promise<any> {
    ...
    // Invalidate all module caches by incrementing version
    await this.cacheInvalidation.incrementModuleVersion();
    console.log(`✅ Module deleted: ${deletedModule.moduleKey} (ID: ${moduleId})`);
